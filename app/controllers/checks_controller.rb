class ChecksController < ApplicationController
  ...
  def show
    ...
    @comment = Comment.new
    @comments = @check.comments.includes(:user).page(params[:page]).per(4).order("created_at DESC")
  end 
  ...
  def search
    @checks_search = Check.search(params[:search]).page(params[:page]).per(4).order("created_at DESC")
  end  

  
  private
  def check_params
    params.require(:check).permit(:pre_score,:chk_score1,:chk_score2,:chk_score3,:chk_score4,:chk_score5,:chk_score,:dif_score)
    .merge(nickname: current_user.nickname, user_id: current_user.id)
  end  
